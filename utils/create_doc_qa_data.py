...
class InputFeatures(object):
    ...
    def __init__(self,
        ...
        self.rank_score = float(rank_score) if rank_score else 1.0

        if self.rank_score < 0.0:
            raise ValueError("rank_score must be positive")
...
class DocInputFeatures(object):
    ...
    def __init__(self, unique_id=None, feature_list=None, num_feature=None,
        ...
        # Computes the denominator for all feature's rank scores.
        self.doc_rank_score = sum([feat.rank_score for feat in feature_list])
...
def uniform_sample(pos_feature_ids, neg_feature_ids, max_num_doc_feature,
    ...
    if single_pos_per_dupe:
        ...
        if np.intersect1d(pos_feature_ids, selected_feature_ids).size < 1:
            tf.logging.info(selected_pos_ids)
...
def rank_score_sample(pos_feature_ids, num_feature, max_num_doc_feature,
                      rank_scores, rank_doc_score):
    """Performs sampling based on rank probabilities from positive and negative samples."""
    required_num_neg = min(max_num_doc_feature - 1, num_feature)

    # First, samples uniformly a positive passage.
    selected_pos_ids = np.random.choice(pos_feature_ids, 1).tolist()

    # Then, samples negatives based on rank scores.
    feature_probs = np.array(rank_scores) / rank_doc_score
    selected_neg_ids = np.random.choice(
        num_feature, required_num_neg, p=feature_probs,
...
class InputFeatureContainer(object):
    ...
    def __init__(self, feature_filename, max_num_doc_feature, batch_size, shuffle_data,
        ...
        if type(feature_filename) == str:
            ...
            # with open(self.feature_filename, mode='r', encoding="utf8") as fin:
            #     self.feature_list = [
            #         DocInputFeatures.load_from_json(line.strip(), keep_topk=topk_for_train)
            #         for line in fin
            #     ]
    ...
    def _process_feature(self, document):
        ...
        for ii, feature in enumerate(document.feature_list):
            # if is_negative(feature.answer_index_list):
            rank_scores.append(feature.rank_score)
            if feature.is_impossible:
        ...
        if self.rank_sample:
            selected_feature_ids = rank_score_sample(
                pos_feature_ids, len(document.feature_list),
                self.max_num_doc_feature, rank_scores, document.doc_rank_score)
        else:
        ...
        for feat_id in selected_feature_ids:
            ...
            if self.is_training:
                ...
                rank_score_list.append(feature.rank_score)
        ...
        return {
            ...
            # 'num_sample': len(unique_ids),
            'num_sample': len(input_ids),
            'rank_score_list': rank_score_list,
            'doc_rank_score': document.doc_rank_score,
            # 'num_unique_ans_str': document.num_answer,
        }
...
class DocFeatureWriter(object):
    """Writes InputFeature to TF example file."""

    def __init__(self, filename, is_training, max_num_doc_feature, rand_seed,
                 allow_null_doc=False, max_num_dupe_factor=6,
                 single_pos_per_dupe=False, pad_dummy_feature=False):
        self.filename = filename
        self.is_training = is_training
        self.num_features = 0
        ...
        np.random.seed(rand_seed)
        self.allow_null_doc = allow_null_doc
        self.max_num_dupe_factor = max_num_dupe_factor
        self.single_pos_per_dupe = single_pos_per_dupe
        self.pad_dummy_feature = pad_dummy_feature

    def process_feature(self, document, null_answer_index=0):
        """Samples pseudo document into multiple examples."""
        if not self.pad_dummy_feature and len(document.feature_list) < self.max_num_doc_feature:
            tf.logging.info(
                "Document has %d features of len %d, less than required %d features" %
                (document.num_feature, len(document.feature_list), self.max_num_doc_feature))
            raise ValueError("Reduces the max_num_doc_feature!")

        is_negative = lambda x: all_null_answer(x, null_answer_index)
        ...
        if self.single_pos_per_dupe:
            self.rng.shuffle(pos_feature_ids)
            self.rng.shuffle(neg_feature_ids)
            required_num_neg = self.max_num_doc_feature - 1
            if len(neg_feature_ids) < required_num_neg:
                selected_feature_ids = pos_feature_ids + neg_feature_ids
            else:
                selected_pos_ids = [pos_feature_ids[0]]
            ...
            selected_feature_ids = selected_feature_ids[:self.max_num_doc_feature]
            self.rng.shuffle(selected_feature_ids)

            if np.intersect1d(pos_feature_ids, selected_feature_ids).size < 1:
                tf.logging.info(selected_pos_ids)
                raise ValueError("Not positive feature is selected")
        else:
            nfeat_sample = min(self.max_num_doc_feature, document.num_feature)
            selected_feature_ids = np.random.choice(
                document.num_feature, nfeat_sample, replace=False)
            while np.intersect1d(pos_feature_ids, selected_feature_ids).size < 1:
                selected_feature_ids = np.random.choice(
                    document.num_feature, nfeat_sample, replace=False)
            selected_feature_ids = list(selected_feature_ids)
        ...
        has_answer = False
        if len(selected_feature_ids) != self.max_num_doc_feature:
            tf.logging.info("pos_feature_ids %s" % pos_feature_ids)
            tf.logging.info("neg_feature_ids %s" % neg_feature_ids)
            tf.logging.info("selected_feature_ids %s" % selected_feature_ids)
            tf.logging.info(
                "selected_feature_ids is smaller than max_num_doc_feature")
            tf.logging.info("selected_feature_ids length %d" %
                            len(selected_feature_ids))
            tf.logging.info("max_num_doc_feature %d" % self.max_num_doc_feature)
        ...
        for feat_id in selected_feature_ids:
            ...
            if self.is_training:
                ...
                answer_index_list.append(
                    feature.answer_index_list)
                notnull_answer_mask.append(
                    [0 if ans_id == null_answer_index else 1
                     for ans_id in feature.answer_index_list])
        ...
        while len(input_ids) < self.max_num_doc_feature:
            # Pads zeros in the end.
            input_ids.append([0 for _ in range(FLAGS.max_seq_length)])
            input_mask.append([0 for _ in range(FLAGS.max_seq_length)])
            segment_ids.append([0 for _ in range(FLAGS.max_seq_length)])

            if is_training:
                start_positions_list.append([0 for _ in range(FLAGS.max_short_answers)])
                end_positions_list.append([0 for _ in range(FLAGS.max_short_answers)])
                answer_positions_mask.append([0 for _ in range(FLAGS.max_short_answers)])
                answer_index_list.append([0 for _ in range(FLAGS.max_short_answers)])
                notnull_answer_mask.append([0 for _ in range(FLAGS.max_short_answers)])

        doc_feature = PseudoDocFeature(
            unique_id=document.unique_id,
            input_ids=sum(input_ids, []),
            input_mask=sum(input_mask, []),
            segment_ids=sum(segment_ids, []),
            start_positions=sum(start_positions_list, []),
            end_positions=sum(end_positions_list, []),
            answer_positions_mask=sum(answer_positions_mask, []),
            answer_ids=sum(answer_index_list, []),
            notnull_answer_mask=sum(notnull_answer_mask, []),
        )

        self.process_doc_feature(doc_feature)

    def process_doc_feature(self, feature):
        """Write a PseudoDocFeature to the TFRecordWriter as a tf.train.Example."""
        self.num_features += 1
        ...
        if self.is_training:
            features["start_positions"] = create_int_feature(feature.start_positions)
            features["end_positions"] = create_int_feature(feature.end_positions)
            features["answer_positions_mask"] = create_int_feature(feature.answer_positions_mask)
            features["answer_ids"] = create_int_feature(feature.answer_ids)
            features["notnull_answer_mask"] = create_int_feature(feature.notnull_answer_mask)
...
def squad_example_generator(input_data, is_training, keep_topk=None,
    ...
    for entry in input_data:
        ...
        for paragraph in entry["paragraphs"]:
            ...
            # Adds rank score for sampling.
            rank_score = float(paragraph.get("passage_rank_score", 1.0))
...
def _pad_document_example(doc_example, example_pool, required_doc_size,
                          sample_chunk_size=200, upper_bound=50):
    """Pads the feature_list of the given _DocSquadExample."""
    example_list = list(doc_example.example_list)
    num_example_to_pad = required_doc_size - len(example_list)

    while num_example_to_pad > 0:
        # Samples a chunk of samples as negatives.
        valid_samples = list(filter(lambda x: x, [
            _convert_example_with_target_example(
                sample, example_list)
            for sample in random.sample(example_pool, sample_chunk_size)
        ]))

        example_list.extend(valid_samples[:num_example_to_pad])

        num_example_to_pad = required_doc_size - len(example_list)

    return _DocSquadExample(
        qid=doc_example.qid, example_list=list(example_list))
...
def read_doc_squad_examples_from_generator(input_file, is_training,
                                           keep_topk=None, pad_example=True):
    ...
    total_num_example = sum(doc_size)
    if pad_example and is_training:
        tf.logging.info("Before padding, there are %d documents" % len(doc_examples))
        tf.logging.info("There are %d examples" % sum(doc_size))
        doc_examples = [_pad_document_example(document, examples, FLAGS.max_num_doc_feature) for document in doc_examples]
        tf.logging.info("After padding, there are %d documents" % len(doc_examples))
        total_num_example = sum([len(item.example_list) for item in doc_examples])
        tf.logging.info("After padding, There are %d examples" % total_num_example)
...
def split_to_words(text):
    return list(re.findall(pat, text))


def roberta_doc_token_processor(orig_doc_tokens, tokenizer, do_lower_case=False):
    """Converts tokens into a list of RoBERTa subtokens."""
    doc_tokens = split_to_words(" ".join(orig_doc_tokens))

    assert len(doc_tokens) >= len(orig_doc_tokens)

    tok_to_orig_index = []
    orig_to_tok_index = []
    all_doc_tokens = []

    # This assumes that example.doc_tokens is shorter.
    start = 0
    for (i, orig_token) in enumerate(orig_doc_tokens):
      if start >= len(doc_tokens):
        raise ValueError("There is no token to be matched!")

      offset = 1
      new_token = ''.join(doc_tokens[start:start + offset])

      # If the current token has a punctunation, it will be split into multiple tokens.
      while new_token.find(orig_token) < 0:
        offset += 1
        new_token = ''.join(doc_tokens[start:start + offset])

      orig_to_tok_index.append(len(all_doc_tokens))
      for token in doc_tokens[start:start + offset]:
        if do_lower_case:
            sub_tokens = tokenizer.tokenize(token.lower())
        else:
            sub_tokens = tokenizer.tokenize(token)
    ...
    if start < len(doc_tokens):
      raise ValueError("There are remaining tokens to be matched!")
...
def convert_examples_to_feature_list_within_doc(
        ...
        is_training, unique_offset, answer_string_to_id, include_title=False,
    ...
    for (example_index, example) in enumerate(examples):
        ...
        if include_title and example.title:
            title_tokens = tokenizer.tokenize(example.title)
            query_tokens = query_tokens + [sep_tok] + title_tokens
...
def convert_doc_examples_to_doc_features(
        ...
        is_training, output_fn, include_title=False, model_type="bert"):
    """Loads a data file into a list of `InputBatch`s."""
    ...
    if model_type == "bert":
        ...
        tf.logging.info("Converts data for RoBERTa")
...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
    ...
    for (example_index, example) in enumerate(examples):
        ...
        for (doc_span_index, doc_span) in enumerate(doc_spans):
            ...
            while len(input_ids) < max_seq_length:
                input_ids.append(0)
                # For roberta, the pad index is 1.
