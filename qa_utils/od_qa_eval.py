...
def read_answers(gold_file):
    ...
    with open(gold_file) as f:
        ...
        for qid, qa in qa_pairs.items():
            answers[qid] = qa["answers"]

    return answers


def evaluate(answers, predictions, skip_no_answer=False, regex_match=False):
    ...
    for qid, ground_truths in answers.items():
        ...
        if regex_match:
            # The EM and F1 are equal for this case.
            regexp = re.compile("(%s)" % ground_truths[0], flags=re.I)
            if re.findall(regexp, prediction):
                exact_match += 1
                f1 += 1
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out_file', type=str, help='metric file')
    args = parser.parse_args()

    answers = read_answers(args.dataset_file)

    # This is hard coded by assuming the qid is informative.
    regex_match = False
    if list(answers.keys())[0].split("_")[0] == "trec":
        print("Using regular expression for answer matching")
        regex_match = True
    ...
    with open(args.out_file, 'wt') as fout:
