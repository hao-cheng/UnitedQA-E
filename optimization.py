...
def create_optimizer(loss, init_lr, num_train_steps, num_warmup_steps,
                     hvd=None, manual_fp16=False, use_fp16=False,
                     num_accumulation_steps=1, optimizer_type="adam",
                     init_loss_scale=2**32, allreduce_post_accumulation=False,
  ...
  opt_opts = optimizer_type.split('-')
  optimizer_type = opt_opts[0]
  power = 1.0
  if len(opt_opts) > 1:
      power = float(opt_opts[1])

  if optimizer_type == "adam" or optimizer_type == "adam_max":
      # power = 1.0
  ...
  if layerwise_lr_decay_power > 0 and optimizer_type == "adam":
    print("Carries out layerwise learning rate decay %f" %
          layerwise_lr_decay_power)
    learning_rate = _get_layer_lrs(learning_rate, layerwise_lr_decay_power,
                                   n_transformer_layers)

  if optimizer_type == "lamb":
      tf.logging.info("Initializing LAMB Optimizer")
      ...
      tf.logging.info("Initializing ADAM Weight Decay Optimizer")
  ...
  elif optimizer_type == "adam_max":
      tf.logging.info("Initializing ADAMMax Weight Decay Optimizer")
      optimizer = AdamMaxWeightDecayOptimizer(
          learning_rate=learning_rate,
          weight_decay_rate=0.01,
          beta_1=0.9,
          beta_2=0.98,
...
class AdamMaxWeightDecayOptimizer(tf.train.Optimizer):
  """A basic Adam optimizer that includes "correct" L2 weight decay."""

  def __init__(self,
               learning_rate,
               weight_decay_rate=0.0,
               spectral_reg_rate=0.0,
               beta_1=0.9,
               beta_2=0.999,
               epsilon=1e-6,
               exclude_from_weight_decay=None,
               name="AdamMaxWeightDecayOptimizer"):
    """Constructs a AdamMaxWeightDecayOptimizer."""
    super(AdamMaxWeightDecayOptimizer, self).__init__(False, name)
  ...
  def apply_gradients(self, grads_and_vars, global_step=None, name=None,
    ...
    for (grad, param) in grads_and_vars:
      ...
      next_v = tf.math.maximum(
          tf.multiply(self.beta_2, v), tf.abs(grad))

      # step = tf.cast(global_step + 1, dtype=tf.float32)
      update = next_m / (next_v + self.epsilon)
      # update /= (1.0 - tf.pow(self.beta_2, step))
...
class AdamWeightDecayOptimizer(tf.train.Optimizer):
  ...
  def _apply_gradients(self, grads_and_vars, learning_rate, manual_fp16=False):
    """See base class."""
    assignments = []
    for (grad, param) in grads_and_vars:
      if grad is None or param is None:
...
def _get_layer_lrs(learning_rate, layer_decay, n_layers, offset=2,
                   decouple_pos_embeddings=False):
  """Have lower learning rates for layers closer to the input."""
  key_to_depths = collections.OrderedDict({
      "/embeddings/": 0,
      "/embeddings_project/": 0,
      "/position_encodings/": n_layers,
      "cls/": n_layers + offset,
  ...
  return {
      key: learning_rate * (layer_decay ** (n_layers + offset - depth))
